...
public class Main {
	...
	private static int daejeon5_2_sum;
	// ※ 전역변수 및 함수 사용 가능합니다.
	...
	static Opponent Me = new Opponent() {
		public int hunt(int opp, int turn, int opp_prev, int opp_last_pattern[][]) {
			...
			if (turn == 0) {
				...
				// 첫 턴에만 상대이전기록 보면서 판단
				for (int i = 0; i < 10; i++) {
					int num = opp_last_pattern[opp][i];
					if (num == 0)
						Dcount++;
					else if (num == 1)
						Rcount++;
				...
				if (Scount >= 6)
					daejeon5_2_sum = 1; // 이런애상대로는 토끼만 빼고 주자,나도 뱀만내면 패널티로 마이너스
			}
			if (daejeon5_2_sum == 1) { // 뱀 많이 쓰는애들한테 토끼 안주기
				if (turn % 2 == 0)
					return 0;
				else
					return 2;
			} else { // 나머지애들 상대로는 토끼 한번 뱀 두번으로 평균점수 올리자
				if (turn % 3 == 0)
					return 1;
				else
					return 2;
